...
-- perdía al refrescar la página o volver a iniciar sesión — mismo hueco
-- que tenían cirugias/tareas_pendientes/seguimientos antes de migrar.
--
-- Notas de diseño:
--
--  · `tipo` y `prioridad` se guardan como el código crudo (ver
--    TIPOS_ORDEN/PRIORIDADES_ORDEN en index.html), NO la etiqueta ni el
--    color ya resueltos — esos se recalculan en el front al reconstruir
--    (construirOrdenDesdeFila), igual que tareas_pendientes.prioridad. No
--    se agregan columnas `tipo_label`/`prioridad_label`/`prioridad_color`
--    para no duplicar un dato derivable.
--
--  · `estado` ('pendiente'/'completado') es la MISMA bandera de dos
--    estados que ya usan Vacunaciones/Desparasitaciones/Documentos: solo
--    la finalización de un Resultado (ver patientData[petKey].resultados,
--    que sigue siendo mock) la mueve a 'completado', y al eliminar ese
--    resultado vuelve a 'pendiente'. Esas dos transiciones también deben
--    persistir esta columna (ver guardarResultado()/rowActionEliminar en
--    index.html) — si no, el estado de la orden quedaría desincronizado
--    del resultado en la próxima recarga.
--
--  · `resultados` (la sub-tabla con los parámetros de laboratorio o los
--    hallazgos de imagen) NO se migra en esta pasada: sigue siendo mock,
--    fuera del alcance de este cambio.
--
--  · Sin policy `_select_red`: una orden no se comparte entre clínicas.
...
-- ══════════════════════════════════════════════════════════════
-- fusionar_mascotas — sumar `ordenes` a c_tablas
-- ══════════════════════════════════════════════════════════════
-- ESTE ARCHIVO PASA A SER LA DEFINICIÓN VIGENTE de la función; supera a
-- 20260809_seguimiento_anestesia.sql (este nombre ordena después a
-- propósito, para que un replay completo de las migraciones no reponga
-- la lista vieja). El cuerpo se tomó de la definición LIVE del proyecto
-- (pg_get_functiondef), no de supabase/schema.sql — schema.sql ya estaba
-- desactualizado en este punto (le faltaban 'anestesia_mediciones' y
-- 'mascota_problemas' en c_tablas, además del bloque de renumeración de
-- mascota_problemas), así que este archivo corrige esa fila y agrega
-- 'ordenes' en el mismo paso.
--
-- Sin esto, unificar dos fichas de la misma mascota perdería EN SILENCIO
-- las órdenes de la ficha duplicada: la fila se borra y el CASCADE se
-- las lleva.
