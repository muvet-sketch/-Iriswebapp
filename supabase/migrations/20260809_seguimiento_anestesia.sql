-- ══════════════════════════════════════════════════════════════
-- SEGUIMIENTO DE ANESTESIA (sub-registro de Cirugías/procedimientos)
-- ══════════════════════════════════════════════════════════════
-- Durante una cirugía con anestesia se monitorea al paciente cada 5–10
-- minutos: FC, FR, PAS/PAD/PAM, SpO2, TC y EtCO2. Hasta ahora eso no
-- tenía dónde vivir: terminaba como prosa dentro de
-- `cirugias.notas_postop`, que no es consultable ni comparable en el
-- tiempo. Cada fila de esta tabla es UNA medición puntual; una anestesia
-- normal produce decenas.
--
-- Notas de diseño:
--
--  · `cirugia_id` con `on delete cascade` es el enlace al registro padre.
--    A diferencia de los seguimientos de hospitalización —que se enganchan
--    por `origen_referencia_id` en texto, `${hospId}:${diaIndex}`, porque
--    un día de kardex no es una fila propia— acá el padre SÍ es una fila
--    con uuid, así que el enlace es una FK de verdad. Borrar el
--    procedimiento se lleva su monitoreo: sin la cirugía, las mediciones
--    no significan nada.
--
--  · `mascota_id` además de `cirugia_id`, aunque sea derivable. Lo exigen
--    tres cosas que ya funcionan así en toda la app: la hidratación de
--    `patientData` resuelve la mascota por esa columna, `fusionar_mascotas`
--    reasigna por esa columna, y el respaldo por establecimiento filtra por
--    `establecimiento_id`.
--
--  · `fecha` es `timestamp` SIN zona horaria, igual que `seguimientos.fecha`
--    y `agenda_eventos.start_iso`. El campo del formulario es un
--    <input type="datetime-local"> y el front trabaja esa cadena local
--    ingenua tal cual; con `timestamptz`, PostgREST la devolvería en UTC
--    con offset y la hora de la medición saldría corrida.
--
--  · Los 8 vitales son nullable y un campo vacío se guarda como NULL.
--    Mismo criterio que el interruptor "No evaluado" del Tablero de
--    trabajo: un vital que no se midió no deja rastro en ninguna parte.
--    Por eso NO hay columnas `*_no_evaluado` ni se escribe el texto
--    "No evaluado" en `observaciones` — y no hay que agregarlas.
--    Consecuencia deliberada: "no se midió" y "el campo nunca se llenó"
--    son indistinguibles.
--
--  · Tipos: `spo2` y `tc` son numeric porque se miden con decimales
--    (98.5 %, 37.4 °C); el resto son enteros. Sin CHECK de rango a
--    propósito: una FC de 300 en un gato bajo anestesia es una urgencia
--    real, no un error de digitación, y la base no es quién para
--    rechazarla.
--
--  · `registrado_por` tiene la misma limitación que
--    `tareas_pendientes.responsable_id` y `seguimientos.registrado_por`:
--    es el id LOCAL numérico de USUARIOS_SISTEMA, que se regenera 1..N en
--    cada sesión, no un uuid estable. Por eso se guarda además el nombre.
--
--  · NO se agrega policy `_select_red` ni un tipo nuevo en
--    `red_solicitudes_tipos_validos`: el monitoreo de anestesia no se
--    comparte entre clínicas por ahora. La cirugía sí se comparte, y eso
--    ya alcanza para que la otra clínica sepa que el procedimiento existió.
...
-- ══════════════════════════════════════════════════════════════
-- fusionar_mascotas — sumar `anestesia_mediciones` a c_tablas
-- ══════════════════════════════════════════════════════════════
-- ESTE ARCHIVO PASA A SER LA DEFINICIÓN VIGENTE de la función; supera a
-- la de 20260809_fusionar_mascotas.sql (el nombre de este archivo ordena
-- después a propósito, para que un replay completo de las migraciones no
-- reponga la lista vieja). El cuerpo es idéntico salvo `c_tablas`.
--
-- Sin esto, unificar dos fichas de la misma mascota perdería EN SILENCIO
-- todo el monitoreo de anestesia de la ficha duplicada: la fila se borra
-- y el CASCADE se lleva sus mediciones.
